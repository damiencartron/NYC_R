...
district3G |> group_by(year) |> 
  summarize(moy = mean(All_tx_34, na.rm = TRUE)) |> 
...
```{r}
district3G_25 |> 
  select(district, Pauv_tx_34, NonPauv_tx_34) |> 
  kable()
```
...
district3G_25 |> 
  tbl_summary(
    include = c(Pauv_tx_34, NonPauv_tx_34),
    type = all_continuous() ~"continuous2",
    statistic = all_continuous2() ~c(
      "{mean} ({sd})",
      "{median} ({p25} ; {p75})",
